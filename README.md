...
Customised for Life3 Biotech.

Using object detection to analyse cell numbers and cell sizes of microalgae on microscopic images
...
- __cookiecutter.project_name:__ Life3 Biotech
- __cookiecutter.description:__ Using object detection to analyse cell numbers and cell sizes of microalgae on microscopic images
- __cookiecutter.repo_name:__ life3-biotech
- __cookiecutter.src_package_name:__ life3_biotech
- __cookiecutter.src_package_name_short:__ life3
- __cookiecutter.gcp_project_id:__ life3-aut0
...
```bash
$ gcloud container clusters get-credentials tekong-exp-cluster --zone asia-southeast1-c --project life3-aut0
```
...
$ polyaxon project create --name life3-biotech-<YOUR_NAME>
```
...
1. Download a service account key to your local machine (or obtain it
   ...
   convention: `aisg-100e-sa@life3-aut0.iam.gserviceaccount.com`.
...
```bash
$ export SA_CLIENT_EMAIL="aisg-100e-sa@life3-aut0.iam.gserviceaccount.com"
...
```bash
$ gcloud container images list --repository=asia.gcr.io/life3-aut0
```
...
`docker/life3-biotech-poly-vscode.Dockerfile` to build a
...
```bash
$ export GCP_PROJECT_ID=life3-aut0
$ docker build \
  -t asia.gcr.io/$GCP_PROJECT_ID/vscode-server:0.1.0 \
  -f docker/life3-biotech-poly-vscode.Dockerfile .
...
```bash
$ polyaxon run -f aisg-context/polyaxon/polyaxonfiles/vscode-service.yml -P DOCKER_IMAGE="asia.gcr.io/$GCP_PROJECT_ID/vscode-server:0.1.0" -P WORKING_DIR='/polyaxon-v1-data' -p life3-biotech-<YOUR_NAME>
```
...
`http://localhost:8888/ui/default/life3-biotech-<YOUR_NAME>/services`
...
$ docker build \
  ...
  -f docker/life3-biotech-jupyter.Dockerfile .
...
```bash
$ polyaxon run -f aisg-context/polyaxon/polyaxonfiles/jupyter-service.yml -P DOCKER_IMAGE="asia.gcr.io/$GCP_PROJECT_ID/jupyter:0.1.0" -P WORKING_DIR="/polyaxon-v1-data" -p life3-biotech-<YOUR_NAME>
```
...
```bash
$ gcloud auth activate-service-account aisg-100e-sa@life3-aut0.iam.gserviceaccount.com --key-file=$GOOGLE_APPLICATION_CREDENTIALS
```
...
$ source ~/.bashrc
$ conda env create -f life3-biotech-conda-env.yml -p /polyaxon-v1-data/workspaces/<YOUR_NAME>/conda_envs/life3-biotech-conda-env
$ alias life3-biotech-conda-env="conda activate /polyaxon-v1-data/workspaces/<YOUR_NAME>/conda_envs/life3-biotech-conda-env"
$ life3-biotech-conda-env
```
...
(`src/life3_biotech/general_utils.py`)
...
a Dockerfile (`docker/life3-biotech-data-prep.Dockerfile`)
provided in this template:

```bash
$ docker build \
  -t asia.gcr.io/$GCP_PROJECT_ID/data-prep:0.1.0 \
  -f docker/life3-biotech-data-prep.Dockerfile .
...
```bash
$ polyaxon run -f aisg-context/polyaxon/polyaxonfiles/process-data.yml -P DOCKER_IMAGE="asia.gcr.io/$GCP_PROJECT_ID/data-prep:0.1.0" -p life3-biotech-<YOUR_NAME> \
  ...
  -P WORKING_DIR="/home/aisg/life3-biotech"
```
...
In the module `src/life3_biotech/general_utils.py`,
...
```bash
$ gsutil mb -p life3-aut0 -c STANDARD -l ASIA-SOUTHEAST1 -b on gs://life3-biotech-artifacts
Creating gs://life3-biotech-artifacts/...
```
...
$ docker build \
  ...
  -f docker/life3-biotech-model-training-gpu.Dockerfile .
...
$ polyaxon run -f aisg-context/polyaxon/polyaxonfiles/train-model-gpu.yml -P DOCKER_IMAGE="asia.gcr.io/$GCP_PROJECT_ID/model-train:0.1.0" -p life3-biotech-<YOUR_NAME> \
...
$ export MLFLOW_TRACKING_PASSWORD=<MLFLOW_TRACKING_PASSWORD>
$ export CLUSTER_IP_OF_MLFLOW_SERVICE=$(kubectl get service/mlflow-nginx-server-svc -o jsonpath='{.spec.clusterIP}' --namespace=polyaxon-v1)
$ polyaxon run -f aisg-context/polyaxon/polyaxonfiles/train-model-gpu-hptuning.yml -P DOCKER_IMAGE="asia.gcr.io/$GCP_PROJECT_ID/model-train:0.1.0" -p life3-biotech-<YOUR_NAME> \
  -P MLFLOW_TRACKING_USERNAME=$MLFLOW_TRACKING_USERNAME -P MLFLOW_TRACKING_PASSWORD=$MLFLOW_TRACKING_PASSWORD \
  -P SETUP_MLFLOW=true -P MLFLOW_AUTOLOG=true \
  -P MLFLOW_TRACKING_URI="http://$CLUSTER_IP_OF_MLFLOW_SERVICE:5005" -P MLFLOW_EXP_NAME=<MLFLOW_EXPERIMENT_NAME> \
  -P WORKING_DIR="/home/aisg/life3-biotech" \
...
directory on GCS:
`gs://life3-biotech-artifacts/mlflow-tracking-server`.
Artifacts for specific runs will be uploaded to a directory with the
following convention:
`gs://life3-biotech-artifacts/mlflow-tracking-server/<MLFLOW_EXPERIMENT_UUID>`.
...
exist more than one package: `life3_biotech` and
`life3_biotech_fastapi`. The former contains the modules for
...
$ export PRED_MODEL_GCS_URI="gs://life3-biotech-artifacts/mlflow-tracking-server/$PRED_MODEL_UUID"
...
```bash
$ cd src
$ gunicorn life3_biotech_fastapi.main:APP -b 0.0.0.0:8080 -w 4 -k uvicorn.workers.UvicornWorker
```
...
`src/life3_biotech_fastapi/config.py`. This class contains
...
$ docker build \
  ...
  -f docker/life3-biotech-fastapi.Dockerfile .
```
...
Dockerfile (`docker/life3-biotech-batch-inferencing.Dockerfile`)
...
```jsonl
...
{"time": "2022-01-06T06:40:27+0000", "filepath": "/home/aisg/life3-biotech/data/1131_2.txt", "logit_prob": 0.006387829780578613, "sentiment": "negative"}
{"time": "2022-01-06T06:40:27+0000", "filepath": "/home/aisg/life3-biotech/data/11020_3.txt", "logit_prob": 0.0041103363037109375, "sentiment": "negative"}
{"time": "2022-01-06T06:40:27+0000", "filepath": "/home/aisg/life3-biotech/data/11916_3.txt", "logit_prob": 0.023626357316970825, "sentiment": "negative"}
{"time": "2022-01-06T06:40:27+0000", "filepath": "/home/aisg/life3-biotech/data/3129_2.txt", "logit_prob": 0.00018364191055297852, "sentiment": "negative"}
{"time": "2022-01-06T06:40:27+0000", "filepath": "/home/aisg/life3-biotech/data/2444_4.txt", "logit_prob": 3.255962656112388e-05, "sentiment": "negative"}
...
```
...
```bash
$ docker build \
  -t asia.gcr.io/life3-aut0/batch-inference:0.1.0 \
  --build-arg PRED_MODEL_UUID="abf043e8a8504eddb1f95bdbc634d2bd" \
  -f docker/life3-biotech-batch-inferencing.Dockerfile .
```
...
$ docker run --rm \
  --env GOOGLE_APPLICATION_CREDENTIALS=/var/secret/cloud.google.com/gcp-service-account.json \
  --env INPUT_DATA_DIR=/home/aisg/life3-biotech/data \
  -v <PATH_TO_SA_JSON_FILE>:/var/secret/cloud.google.com/gcp-service-account.json \
  -v $PWD/models:/home/aisg/from-gcs \
  -v $PWD/outputs:/home/aisg/life3-biotech/outputs \
  -v <PATH_TO_DIR_CONTAINING_TXT_FILES>:/home/aisg/life3-biotech/data \
  asia.gcr.io/life3-aut0/batch-inference:0.1.0
```
...
pages:
  ...
  script:
  - conda env update -f life3-biotech-conda-env.yml
  - conda init bash
  - source ~/.bashrc
