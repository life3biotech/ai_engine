...
RUN conda install python=3.6.8 cudatoolkit=10.1 cudnn=7.6.4
COPY requirements_train_gpu.txt requirements_train_gpu.txt
RUN pip3 install -r requirements_train_gpu.txt --timeout 100 --no-cache-dir
