...
COPY requirements_infer.txt requirements_infer.txt
RUN pip3 install -r requirements_infer.txt --timeout 100 --no-cache-dir

COPY src src
# copy any other files here

ENV PYTHONPATH "${PYTHONPATH}:${CONDA_PATH}:${WORK_DIR}/src/life3_biotech/modeling/EfficientDet"
CMD [ "python3", "-m" , "src.batch_inferencing"]
