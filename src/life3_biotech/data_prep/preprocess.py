...
class Preprocessor:
    ...
    def split_data(self, concatenated_df: pd.DataFrame, test_size=0.2, val_size=0.1):
        ...
        if const.STRATIFY_COLUMN == "None":
            self.logger.info("Splitting data without stratification")
            ...
            self.logger.info(f"Splitting data with stratification by {const.STRATIFY_COLUMN}")
        ...
        if const.SAVE_DATA_SPLITS:
            ...
            self.logger.info("Saved datasets to files")
