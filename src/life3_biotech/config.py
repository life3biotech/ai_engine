...
class PipelineConfig:
    ...
    def __init__(self, params, logger):
        ...
        if "train" in params:
            ...
            const.EVAL_CELL_ACCU_AS_CELL = train_params["eval_cell_accu_as_cell"]
        ...
        if "inference" in params:
            ...
            # Defined cell size
            const.UM_PIXEL_MAPPING = inf_params["um_pixel_mapping"]
            const.SMALL_MID_CELL_CUTOFF = inf_params["small_mid_cell_cutoff"]
            const.MID_LARGE_CELL_CUTOFF = inf_params["mid_large_cell_cutoff"]
            const.USE_CALIBRATED_CELLSIZE = inf_params["use_calibrated_cellsize"]
            # Input/Output
            const.INFERENCE_MODEL_PATH = inf_params["model_path"]
            const.IMAGE_INPUT_DIR = inf_params["image_input_dir"]
            const.CSV_OUTPUT_DIR = inf_params["csv_output_dir"]
            const.SAVE_OUTPUT_IMAGE = inf_params["save_output_image"]
            const.SAVE_OUTPUT_IMAGE_SHOWLABEL = inf_params[
                "save_output_image_showlabel"
            ]
            const.SAVE_OUTPUT_IMAGE_SHOW_CELLCOUNT = inf_params[
            ...
            const.POSTPROCESS_BBOX_SORT = inf_params["postprocess_bbox_sort"]
            ...
            const.INFERENCE_SLICE = inf_params["inference_slice"]
        ...
        if "efficientdet" in params:
            ...
            if "max_detections" in ed_params:
                const.MAX_DETECTIONS = ed_params["max_detections"]
            if "class_specific_filter" in ed_params:
                const.CLASS_SPECIFIC_FILTER = ed_params["class_specific_filter"]
            if "detect_quadrangle" in ed_params:
                const.DETECT_QUADRANGLE = ed_params["detect_quadrangle"]
            if "score_threshold" in ed_params:
                const.SCORE_THRESHOLD = ed_params["score_threshold"]
            if "select_top_k" in ed_params:
                const.SELECT_TOP_K = ed_params["select_top_k"]
