...
def draw_boxes(image, boxes, scores, labels, colors, classes):
    for b, l, s in zip(boxes, labels, scores):
        ...
        score = f'{round(s*100,2)}%'
        color = colors[class_id]
        label = '-'.join([class_name, score])
    
        ret, baseline = cv2.getTextSize(label, cv2.FONT_HERSHEY_DUPLEX, 1.0, 1)
        ...
        image = cv2.putText(image, label, (xmin, ymax - baseline), cv2.FONT_HERSHEY_DUPLEX, 1.0, (0, 0, 0), 1)
    return image
