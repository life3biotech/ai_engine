...
def confluence_nms(
    bounding_boxes, scores, classes, confluence_thr, gaussian, score_thr=0.05, sigma=0.5
):
    """
    Parameters:
       bounding_boxes: list of bounding boxes (x1,y1,x2,y2)
    ...
    for each_class in class_mapping:
        ...
        while dets.size > 0:
            ...
            x1, y1, x2, y2 = normalise_coordinates(
                x1, y1, x2, y2, min_x, max_x, min_y, max_y
            )
            ...
            if gaussian == True:
                gaussian_weights = np.exp(
                    -((1 - manhattan_distance) * (1 - manhattan_distance)) / sigma
                )
                weights[manhattan_distance <= confluence_thr] = gaussian_weights[
                    manhattan_distance <= confluence_thr
                ]
            else:
                weights[manhattan_distance <= confluence_thr] = manhattan_distance[
                    manhattan_distance <= confluence_thr
                ]
    ...
    for each_class in class_mapping:
        ...
        while dets.size > 0:
            ...
            while len(confluence_scores) < np.size(dets, 0):
                ...
                xx1, yy1, xx2, yy2 = normalise_coordinates(
                    xx1, yy1, xx2, yy2, min_x, max_x, min_y, max_y
                )
                ...
                all_proximities[proximity <= confluence_thr] = proximity[
                    proximity <= confluence_thr
                ]
                cconf_scores[proximity <= confluence_thr] = cconf[
                    proximity <= confluence_thr
                ]
                ...
                if all_proximities.size > 0:
                    proximity = (sum(all_proximities) / all_proximities.size) * (
                        1 - confidence_score
                    )
                else:
