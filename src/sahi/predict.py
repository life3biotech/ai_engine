...
def get_prediction(
    ...
    detection_model.perform_inference(
        np.ascontiguousarray(image_as_pil), image_size=image_size
    )
...
def get_sliced_prediction(
    ...
    #     )
        ...
        raise ValueError(
            f"postprocess_type should be one of ['NMS', 'NMM', 'GREEDYNMM', 'CONFLUENCE'] but given as {postprocess_type}"
        )

    # create prediction input
    # print("slice_image_result:", slice_image_result)
    # print("slice_image_result len:", len(slice_image_result))
...
def predict(
    ...
    for ind, image_path in enumerate(
        tqdm(image_path_list, "Performing inference on images")
    ):
        # get filename
