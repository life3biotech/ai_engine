...
def visualize_prediction(
    ...
    for i in range(len(boxes)):
        ...
        label = f"{class_}"
        w, h = cv2.getTextSize(label, 0, fontScale=text_size, thickness=text_th)[
            0
        ]  # label width, height
...
def visualize_object_predictions(
    ...
    # print cellcount info on image
    if show_cellcount:
        w, h = draw_text(
            image, f" Total cells: {cellcount_info.cell_tot[0]} ", pos=(10, 10)
        )
        draw_text(
            image,
            f" Small cells: {cellcount_info.tot_small_cell[0]} ",
            # font_scale=4,
            pos=(10, 10 + h),
        ...
        draw_text(
            image,
            f" Medium cells: {cellcount_info.tot_mid_cell[0]} ",
            # font_scale=4,
            pos=(10, 10 + 2 * h),
            # text_color_bg=(255, 0, 0),
        )
        draw_text(
            image,
            f" Large cells: {cellcount_info.tot_large_cell[0]} ",
            # font_scale=4,
            pos=(10, 10 + 3 * h),
            # text_color_bg=(255, 0, 0),
        )
