...
from life3_biotech.modeling.EfficientDet.confluence import confluence, confluence_nms
...
def nms(
    ...
    # according to their confidence scores or box area
    if const.POSTPROCESS_BBOX_SORT:
...
def nmm(
    ...
    for ind in range(len(object_predictions_as_tensor)):
        ...
        # keep the boxes with IoU/IoS less than thresh_iou
        mask = match_metric_value < match_threshold
        matched_box_indices = other_pred_inds[(mask == False).nonzero().flatten()].flip(
            dims=(0,)
        )
